...
def svm(x_train: list, x_test: list, y_train: list, y_test: list, ngram: int = 1, use_stoplist: bool = False) -> int:
    ...
    if (use_stoplist):
        ...
        model = make_pipeline(TfidfVectorizer(stop_words=stoplist, ngram_range=(ngram, ngram)), LinearSVC())
    else: model = make_pipeline(TfidfVectorizer(ngram_range=(ngram, ngram)), LinearSVC())
